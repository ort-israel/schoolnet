...
// Lea 2016/07/24 - check for redirect parameter 
$redirect_to = optional_param('redirectto', '', PARAM_TEXT); // page to redirect to
...
if (isloggedin() && $errorcode == 0){ // Lea 2016/0/ - It's not enough to check isloggedin() because it's true even when there is an error in the log in process. Checking $errorcode adds what we need.
// take the user to the redirected page. If none is specified, take him to the course id
	$goto_page = $CFG->wwwroot;
	echo $redirect_to;
	if (!empty($redirect_to)){
		$goto_page .= $redirect_to;
	}
	else{
		$goto_page .= '/course/view.php?id='.$courseid;
	}
    redirect($goto_page);
}else{ // user isn't logged in, redirect them to regulat HTML login page
    redirect('http://mapa-moodle27.ort.org.il/hashmal_login/index.html?error=1'); 
}
